...
import { Send, Heart, Sparkles, Check, Mail, Sun, Smile, Lock } from 'lucide-react';

interface MessageFormProps {
  onSendMessage: (senderName: string, message: string, mood: string) => Promise<{ success: boolean; targetEmail?: string }>;
}
...
export const MessageForm: React.FC<MessageFormProps> = ({ onSendMessage }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Send to server API endpoint (which logs message & tracks target email vatsalpatelwork20@gmail.com)
      await onSendMessage(senderName, message, selectedMood);
      
      // Also construct mailto URL as a instant fallback/direct email dispatch option
      const mailtoSubject = encodeURIComponent(`Love Note from ${senderName} 🌻`);
      const mailtoBody = encodeURIComponent(
        `Hi Vatsal!\n\nHere is a new message written from our Sunflower Website:\n\n` +
        `Mood: ${selectedMood}\n` +
        `Message:\n${message}\n\n` +
        `With Love,\n${senderName}`
      );
      
      // Attempt opening mailto link silently in background or as fallback
      window.open(`mailto:vatsalpatelwork20@gmail.com?subject=${mailtoSubject}&body=${mailtoBody}`, '_blank');
  ...
  return (
    <section id="message-section" className="py-12 md:py-20 px-4 max-w-3xl mx-auto">
      ...
      <div className="bg-[#FFFDF6] border-4 border-[#CA8A04] rounded-3xl p-6 sm:p-10 polaroid-shadow relative overflow-hidden">
        ...
        <div className="washi-tape-pink absolute -top-4 right-8 px-6 py-1 font-handwritten text-xs text-pink-900">
        ...
        <div className="text-center my-4">
          ...
          <p className="font-handwritten text-xl sm:text-2xl text-[#92400E] mt-1">
            Write anything you want! It will be sent directly to Vatsal's inbox & saved for him 💛
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-6">
          ...
          {/* Email Info Badge */}
          <div className="flex items-center gap-2 bg-amber-50 border border-amber-200 p-3 rounded-xl text-xs font-cute text-amber-800">
            <Mail className="w-4 h-4 text-amber-600 shrink-0" />
            <span>
              Destination Email: <strong className="text-amber-950">vatsalpatelwork20@gmail.com</strong>
            </span>
          </div>
      ...
      <AnimatePresence>
        {sentSuccess && (
          ...
          >
            ...
            >
              <div className="w-16 h-16 rounded-full bg-emerald-100 border-2 border-emerald-400 text-emerald-600 mx-auto flex items-center justify-center text-3xl mb-4">
                🌻
              </div>

              <h3 className="font-cute text-3xl font-bold text-amber-900">
                Message Sent To Vatsal! 💖
              </h3>

              <p className="font-handwritten text-2xl text-amber-800 mt-2">
                Thank you so much! Your note was sent to <strong className="text-amber-950">vatsalpatelwork20@gmail.com</strong> and saved for Vatsal to read!
              </p>
