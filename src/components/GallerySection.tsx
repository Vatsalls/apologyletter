...
import { Heart, Plus, Sparkles, X, Upload, Image as ImageIcon, Check, Filter } from 'lucide-react';
...
export const GallerySection: React.FC<GallerySectionProps> = ({
  ...
  const handleCreatePhoto = (e: React.FormEvent) => {
    e.preventDefault();
    const finalUrl = filePreview || newUrl || 'https://images.unsplash.com/photo-1597848212624-a19eb35e2651?auto=format&fit=crop&w=1000&q=80';
    
    const created: PhotoItem = {
      id: 'photo-' + Date.now(),
      url: finalUrl,
      caption: newCaption || 'Our Special Sunflower Memory 💛',
      date: 'Today',
  ...
  const filteredPhotos = filterStyle === 'all' 
    ? photos 
    : photos.filter(p => p.frameStyle === filterStyle);
  ...
  return (
    <section id="gallery-section" className="py-12 md:py-20 px-4 max-w-6xl mx-auto">
      ...
      <div className="text-center mb-10">
        <div className="inline-blockWas border-2 border-dashed border-amber-400 bg-[#FEF08A]/70 px-6 py-1.5 rounded-full mb-3">
          <span className="font-handwritten text-xl font-bold text-amber-900 flex items-center gap-2 justify-center">
            <span>📷</span> Cute Photo Memories Gallery <span>🌻</span>
          </span>
        </div>
        
        <h2 className="font-cute text-3xl sm:text-5xl font-bold text-[#78350F]">
          Our Love In Sunflowers & Frames
        </h2>
        
        <p className="font-handwritten text-xl sm:text-2xl text-[#92400E] max-w-xl mx-auto mt-2">
          Every photo is framed with hand-drawn doodle borders, cute tape stickers, and warm love.
        </p>

        {/* Action Controls: Add Photo & Filter */}
        <div className="flex flex-wrap items-center justify-center gap-3 mt-6">
          ...
          >
            ...
            <span>Add Our Memory Photo 📸</span>
          </button>

          {/* Filter Options */}
          <div className="flex items-center gap-1 bg-white/80 p-1 rounded-full border border-amber-200 shadow-xs font-cute text-xs">
            <Filter className="w-3.5 h-3.5 text-amber-700 ml-2" />
            <button
              onClick={() => setFilterStyle('all')}
              className={`px-3 py-1 rounded-full transition-colors ${filterStyle === 'all' ? 'bg-amber-300 text-amber-900 font-bold' : 'text-amber-800 hover:bg-amber-100'}`}
            >
              All
            </button>
            <button
              onClick={() => setFilterStyle('polaroid')}
              className={`px-3 py-1 rounded-full transition-colors ${filterStyle === 'polaroid' ? 'bg-amber-300 text-amber-900 font-bold' : 'text-amber-800 hover:bg-amber-100'}`}
            >
              Polaroid
            </button>
            <button
              onClick={() => setFilterStyle('sunflower')}
              className={`px-3 py-1 rounded-full transition-colors ${filterStyle === 'sunflower' ? 'bg-amber-300 text-amber-900 font-bold' : 'text-amber-800 hover:bg-amber-100'}`}
            >
              Sunflower
            </button>
            <button
              onClick={() => setFilterStyle('tape')}
              className={`px-3 py-1 rounded-full transition-colors ${filterStyle === 'tape' ? 'bg-amber-300 text-amber-900 font-bold' : 'text-amber-800 hover:bg-amber-100'}`}
            >
              Tape
            </button>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 sm:gap-10">
        {filteredPhotos.map((photo, index) => (
          ...
          >
            ...
            <div 
              className="overflow-hidden rounded-md border border-amber-200 cursor-pointer aspect-4/3 relative group"
            ...
            >
              ...
              <div className="absolute inset-0 bg-amber-900/30 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center text-white font-cute font-bold text-sm gap-1">
                <Sparkles className="w-4 h-4" /> Tap to view memory
              </div>
            </div>
          ...
          >
            ...
            >
              ...
              <div className="washi-tape-yellow mx-auto mb-4 px-8 py-1 font-handwritten text-sm text-amber-900 inline-block">
                Sunflower Polaroid View 🌻
              </div>

              <div className="rounded-2xl overflow-hidden border-2 border-amber-300 max-h-[60vh] flex items-center justify-center bg-black/5">
                <img
            ...
            >
              ...
              <div className="text-center mb-6">
                ...
                <h3 className="font-cute text-2xl font-bold text-amber-900">
                  Add A New Sunflower Memory
                </h3>
                <p className="font-handwritten text-lg text-amber-700">
                  Upload a photo from your phone/device or paste an image link!
                </p>
              </div>
              ...
              <form onSubmit={handleCreatePhoto} className="space-y-4">
                ...
                {filePreview ? (
                  <div className="relative w-32 h-32 mx-auto rounded-xl overflow-hidden border-2 border-amber-400">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full p-2.5 bg-white border border-amber-300 rounded-xl font-cute text-sm text-amber-900 focus:outline-none focus:ring-2 focus:ring-amber-400"
                    />
                  </div>
                )}
                ...
                <div>
                  ...
                  <input
                    type="text"
                    required
                    placeholder="e.g. Happiness is being with you 🌻"
                    value={newCaption}
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div>
                    ...
                    >
                      ...
                      <option value="💖">💖 Pink Heart</option>
                      <option value="✨">✨ Sparkles</option>
                      <option value="🐝">🐝 Honeybee</option>
                      <option value="💌">💌 Love Note</option>
                    </select>
                  </div>
                </div>
